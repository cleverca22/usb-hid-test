  ...
  HID_COLLECTION(HID_COLLECTION_APPLICATION),
    HID_COLLECTION(HID_COLLECTION_LOGICAL),
      ...
      HID_PUSH,
        ...
        HID_REPORT_SIZE(1),
        HID_LOGICAL_MAX(1),  // each button can return a value between 0 and 1
        HID_PHYSICAL_MAX(1),
        HID_USAGE_PAGE(HID_USAGE_PAGE_BUTTON),
        HID_USAGE_MIN(1),   // this is decribing buttons 1 thru 8
...
int main(int argc, char **argv) {
  ...
  write(fd, hid_descriptor, sizeof(hid_descriptor));
  close(fd);
  for (int i=0; i<sizeof(hid_descriptor); i++) {
    printf("0x%x,", hid_descriptor[i]);
  }
  puts("");
  return 0;
}
